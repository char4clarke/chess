    ...
    public <R> TypeAdapter<R> create(Gson gson, TypeToken<R> type) {
        ...
        return new TypeAdapter<R>() {
            ...
            public void write(JsonWriter out, R value) throws IOException {
                ...
                if (maintainType) {
                    JsonElement existingType = jsonObject.get(typeFieldName);
                    if (existingType != null) {
                        if (!existingType.getAsString().equals(label)) {
                            throw new JsonParseException("Conflicting type name: " + existingType.getAsString());
                        }
                    } else {
